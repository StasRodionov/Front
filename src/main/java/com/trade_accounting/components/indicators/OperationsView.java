...
import com.trade_accounting.components.goods.TitleForModal;
...
public class OperationsView extends VerticalLayout {
    ...
    private HorizontalLayout configSubMenu() {
        ...
        salesSubCustomersOrders.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("salesSubCustomersOrdersModal")));
        ...
        salesSubInvoicesToBuyers.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("salesSubInvoicesToBuyersModal")));

        MenuItem salesSubShipment = operationsSubMenu.addItem("Отгрузки");
        salesSubShipment.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("salesSubShipmentModal")));

        //Закупки
        MenuItem supplierOrders = operationsSubMenu.addItem("Заказы поставщикам");
        supplierOrders.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("supplierOrdersModal")));

        MenuItem vendorAccounts = operationsSubMenu.addItem("Счета поставщиков");
        vendorAccounts.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("vendorAccountsModal")));

        MenuItem admissions = operationsSubMenu.addItem("Приемка");
        admissions.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("admissionsModal")));

        //Деньги

        MenuItem incomingPaymentModal = operationsSubMenu.addItem("Входящие платежи");
        incomingPaymentModal.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("incomingPaymentModal")));

        MenuItem сreditOrder = operationsSubMenu.addItem("Приходные ордеры");
        сreditOrder.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("сreditOrderModal")));

        MenuItem outComingPaymentModal = operationsSubMenu.addItem("Исходящие платежи");
        outComingPaymentModal.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("outComingPaymentModal")));

        MenuItem expenseOrder = operationsSubMenu.addItem("Расходные ордеры");
        expenseOrder.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("expenseOrderModal")));

        //Склад
        MenuItem internalOrder = operationsSubMenu.addItem("Внутренний заказ");
        internalOrder.addClickListener(e -> operations.getUI()
                .ifPresent(ui -> ui.navigate("internalOrderModal")));
        ...
        MenuItem inventory = operationsSubMenu.addItem("Инвентаризация");
        inventory.addClickListener(e -> operations.getUI().ifPresent(ui -> ui.navigate("goods/inventory")));

        MenuItem posting = operationsSubMenu.addItem("Оприходование");
        posting.addClickListener(e -> operations.getUI().ifPresent(ui -> ui.navigate("postingModal")));

        MenuItem loss = operationsSubMenu.addItem("Списание");
        loss.addClickListener(e -> operations.getUI().ifPresent(ui -> ui.navigate("lossModal")));
