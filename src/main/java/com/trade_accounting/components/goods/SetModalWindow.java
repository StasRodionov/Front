...
@UIScope
@Slf4j
public class SetModalWindow extends Dialog {
    ...
    @Autowired
    public SetModalWindow(ProductPriceService productPriceService, UnitService unitService,
        ...
        nameTextField.setPlaceholder("Наименование комплекта");
        ...
        descriptionField.setPlaceholder("Описание комплкта");
        ...
        productGroupDtoComboBox.setPlaceholder("Товарная группа");
        ...
        unitDtoComboBox.setPlaceholder("Единица измерения");
        ...
        weightNumberField.setPlaceholder("Вес");
        productDtoBinder.forField(weightNumberField)
                .withValidator(Objects::nonNull, "Введите вес")
                .withValidator(new BigDecimalRangeValidator("Не верное значение", BigDecimal.ZERO, new BigDecimal("99999999999999999999")))
        ...
        volumeNumberField.setPlaceholder("Объём");
        ...
        taxSystemDtoComboBox.setPlaceholder("Система налогообложения");
        ...
        attributeOfCalculationObjectComboBox.setPlaceholder("Предмет расчета");
    ...
    private Component getHeader() {
        ...
        H2 title = new H2("Добавление комплекта");
    ...
    private Button getAddButton() {
        return new Button("Добавить", event -> {
            ...
            if (!productDtoBinder.validate().isOk() || !priceDtoBinder.validate().isOk()) {
                ...
                Notification.show(String.format("Комплект %s добавлен", productDto.getName()));
                close();
            }
        });
    }
    ...
    private Button getRemoveButton(ProductDto productDto) {
        Button deleteButton = new Button("Удалить", buttonClickEvent -> {
            ...
            Notification.show(String.format("Комплект %s удален", productDto.getName()));
            close();
        });
    ...
    private Button getUpdateButton(ProductDto productDto) {
        return new Button("Изменить", event -> {
                ...
                Notification.show(String.format("Комплект %s изменен", productDto.getName()));
                close();
